...
def getSQL(ident, table, userid):
	cmd = 'twurl ' 
	if (userid):
		cmd = cmd + '/1/users/show.json?user_id=' + ident
	else:
		cmd = cmd + '/1/users/show.json?screen_name=' + ident
	...
	try: 
		...
		for line in twitter:
			try:
				checkRate()
				print line.rstrip('\r\n')
				saveSQL(getSQL(line.rstrip('\r\n'),'candidatestats', False))
			except:
				print sys.exc_info()[0]
				appendDailyLog("ID - " + ID + " : Error >> " + sys.exc_info()[0])
	except:
		print "Error >> Unable to capture error"
	print 'Completed'
